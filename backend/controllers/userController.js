...
const getUsersByFirstName = asyncHandler(async (req, res) => {
  ...
  if (users && !(Object.keys(users).length === 0)) {
    ...
    res.status(404);
    throw new Error(
      `Users with first name ${req.params.firstName} are not found !`
    );
  }
});
...
const getUsersByLastName = asyncHandler(async (req, res) => {
  ...
  if (users && !(Object.keys(users).length === 0)) {
    ...
    res.status(404);
    throw new Error(
      `Users with last name ${req.params.lastName} are not found !`
    );
  }
});
...
const getUserByEmail = asyncHandler(async (req, res) => {
  let user = await getUserByEmailFromDB(req.params.email);
...
const updateUser = asyncHandler(async (req, res) => {
  ...
  let updatedUserWithHelper = UserHelper.updateUserHelper(
    userToUpdate,
    req.body
  );
